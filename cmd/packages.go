...
func startWorker(userInput string) <-chan error {
	...
	go func() {
		...
		if appConfig.Anchore.UploadEnabled {
			if err := upload(src, src.Metadata, catalog, d); err != nil {
				errs <- err
				return
			}
		}
		...
		bus.Publish(partybus.Event{
			...
			Value: presenter.GetPresenter(appConfig.PresenterOpt, src.Metadata, catalog, d),
		})
	}()
	return errs
}

func upload(src source.Source, s source.Metadata, catalog *pkg.Catalog, d *distro.Distro) error {
